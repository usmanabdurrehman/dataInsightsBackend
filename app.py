...
@app.route('/')
def index():
	return 'This is the DataInsights.io server'
...
def getData():
	...
	if(showTicks['custom']):
		...
		df = pd.read_csv(BASE_PATH + 'example_datasets/heart.csv')
		target = 'target'
	elif(showTicks['prostate']):
		filename = 'prostate dataset'
		df = pd.read_csv(BASE_PATH + 'example_datasets/prostate.csv')
		target = 'lpsa'
	else:
		filename = 'Stock Prices dataset'
		df = pd.read_csv(BASE_PATH +'example_datasets/stock_prices.csv')
		target = 'Stock price'
